...
#define DBG_DCHECK_LT(a, b)             DBG_DCHECK((a) < (b))
#define DBG_DCHECK_GE(a, b)             DBG_DCHECK((a) >= (b))
#define DBG_DCHECK_GT(a, b)             DBG_DCHECK((a) > (b))

// Use this macro to verify at compile time that certain restrictions are met.
// The argument is the boolean expression to evaluate.
// Example:
//   DBG_COMPILE_ASSERT(sizeof(foo) < 128);
// Note: In C++, use static_assert instead!
#define DBG_COMPILE_ASSERT(expression) switch (0) {case 0: case expression:;}
...
// =============================================================================
static int16_t WebRtcVad_FindMinimum(VadInstT* self,
...
static int WebRtcVad_InitCore( VadInstT* self );
...
void *vad_start( void ){
    ...
    if( ( tp_vad = malloc( sizeof(VadInstT) ) ) == NULL ){
#ifdef OPEN_DBUG
        DLLOGV( "vad.start.request mem failed!!" );
#endif
        return NULL;
    }
    WebRtcVad_InitCore( tp_vad );
    WebRtcVad_set_mode_core( tp_vad, 3 );
    return tp_vad;
}

int vad_close( void *ivad ){
    if( ivad == NULL ) return -1;
    free( ivad );
    return 0;
}


int vad_running_16k( void *ivad, const int16_t *iindata, unsigned idlen ){
    static int  tv_flag = 0;
    VadInstT    *tp_vad;
    int16_t     tv_buf[ 240 ];
...
#else
    int         tv_vadvalue;
    tv_vadvalue = WebRtcVad_CalcVad8khz( tp_vad, tv_buf, idlen/2 );
#endif
    if( tp_vad->vad ){
        if( tv_flag == 0 ) tv_ret = e_vad_ret_turn_activation;
        else tv_ret = e_vad_ret_is_activation;
        tv_flag = 40;
    }else{
        if( tv_flag > 1 ) tv_ret = e_vad_ret_is_activation;
        else if( tv_flag == 1 ) tv_ret = e_vad_ret_turn_inactivation;
        else tv_ret = e_vad_ret_is_inactivation;
        if( tv_flag ) tv_flag--;
    }
    return tv_ret;
}
...
#ifdef OPEN_DBUG
    DLLOGE( "#" );
    DLLOGE( "# Faltal error in %s, line %d", file, line );
    DLLOGE( "# last system error: %d", errno );
    DLLOGE( "# %d", msg );
    DLLOGE( "#" );
#endif
    return;
...
static int16_t WebRtcVad_FindMinimum(VadInstT* self,
    ...
    DBG_DCHECK_LT(channel, kNumChannels);
...
#if 1
    int16_t *sptrend = in_vector + in_vector_length;
    while( sptr < sptrend ){
...
static int16_t WebRtcVad_CalculateFeatures(VadInstT* self, const int16_t* data_in,
...
static int16_t GmmProbability(VadInstT* self, int16_t* features,
    ...
    // DLLOGW("%d - %d", vadflag, self->over_hang);
